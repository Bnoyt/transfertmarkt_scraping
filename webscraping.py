# -*- coding: utf-8 -*-
"""
Created on Sun Feb 11 21:04:12 2018
...
os.chdir("C:/Users/Benoit/Desktop/Travail/Polytechnique/Informatique/INF471G - graphe global géant/Projet")
...
class Club(object):
	...
	def save(self):
		...
		a = open("clubs/" + titre.split(' -')[0] + ".html","wb")
		a.write(self.html)
		a.close()		
...
class Player(object):
	...
	def save(self):
		soup = BeautifulSoup(self.html)
		titre = soup.find("title").text
		a = open("joueurs/" + titre.split(' -')[0] + ".html","wb")
		a.write(self.html)
		a.close()
...
def getLigues():
	url= "https://www.transfermarkt.fr/wettbewerbe/europa/"
	headers = {"User-Agent":"Mozilla/5.0"}
	requete = requests.get(url, headers=headers)
	page = requete.content
	...
	uris = []
	for t in tableau:
		uris.append("https://www.transfermarkt.fr" + t.find_all("a")[1]["href"])
	...
	for t in tableau:
		...
		if len(temp) > 3:
			if "*" not in temp[3].text:
				uris.append("https://www.transfermarkt.fr" + temp[3]["href"])
		else:
...
def extract_club_data(html):
	...
	ligue = nettoyer(soup.find("span",class_="hauptpunkt").text)[32:]
	pays = soup.find("span",class_="mediumpunkt").find("img")["alt"]
...
def extract_player_data(html):
	...
	for saison in palmares_text:
		...
		if annee >= 1980:
			trophes[annee-1980] += 1
	return [uri_player,nom_joueur,nom_origine_joueur,date_de_naissance,nationalite,position,club_actuel,uri_club,valeur] + trophes
...
def extract_all_player_data():
	...
	for i in range(A,B):
		...
		if A%100 == 0:
			...
			t1 = t2
		a = open("joueurs/" + fichier_joueur,"rb")
		...
		if i%2 == 0:
			...
			if len(body) > 0:
				...
				for l in lignes:
					...
					uri_joueur = "http://www.transfermarkt.fr" + html_joueur["href"]
					nom_joueur = html_joueur.text
					prix = html_prix.text
					target = uri_base
					source = "http://www.transfermarkt.fr" + html_club["href"].split("transfers")[0]
					nom_dest = nom_base
			...
			if len(body) > 0:
				...
				for l in lignes:
					...
					prix = html_prix.text
					source = uri_base
					target = "http://www.transfermarkt.fr" + html_club["href"].split("transfers")[0]
					nom_source = nom_base
	...
	for l in ligues:
		...
		for i in range(1980,2017):
			...
			for k in range(len(entrees)):
				...
				uri_club = "http://www.transfermarkt.fr" + a_club["href"].split("spielplan/")[0]
